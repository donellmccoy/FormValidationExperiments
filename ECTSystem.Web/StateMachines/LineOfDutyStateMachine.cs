...
/// <see cref="WorkflowTrigger"/> actions and persists transition side-effects (workflow state
...
internal class LineOfDutyStateMachine
{
    ...
    /// between <see cref="WorkflowState"/> values triggered by <see cref="WorkflowTrigger"/> actions.
    /// </summary>
    private readonly StateMachine<WorkflowState, WorkflowTrigger> _sm;

    /// <summary>
    /// Parameterized trigger for <see cref="WorkflowTrigger.Return"/> that carries
    ...
    /// </summary>
    private readonly StateMachine<WorkflowState, WorkflowTrigger>.TriggerWithParameters<LineOfDutyCase, WorkflowState> _returnTrigger;
    ...
    /// back to the <see cref="FireAsync(WorkflowTrigger)"/> and
    /// <see cref="FireAsync(LineOfDutyCase, WorkflowTrigger)"/> methods.
    /// </summary>
    ...
    /// <summary>
    /// Asynchronously retrieves the set of <see cref="WorkflowTrigger"/> values that can
    ...
    public async Task<IEnumerable<WorkflowTrigger>> GetPermittedTriggersAsync()
    {
    ...
    /// </param>
    private async Task HandleTransitionAsync(StateMachine<WorkflowState, WorkflowTrigger>.Transition transition)
    {
        try
        {
    ...
    public LineOfDutyStateMachine(LineOfDutyCase lineOfDutyCase, IWorkflowHistoryService historyService)
    {
        ...
        _sm = new StateMachine<WorkflowState, WorkflowTrigger>(lineOfDutyCase.GetCurrentWorkflowState(), FiringMode.Queued);

        _returnTrigger = _sm.SetTriggerParameters<LineOfDutyCase, WorkflowState>(WorkflowTrigger.Return);
    ...
    /// scratch; call <see cref="FireAsync(LineOfDutyCase, WorkflowTrigger)"/> with
    /// <see cref="WorkflowTrigger.ForwardToMemberInformationEntry"/> to advance past the draft state.
    /// </summary>
    /// <param name="historyService">
    /// The workflow history service used to persist history entries during transitions.
    /// </param>
    public LineOfDutyStateMachine(IWorkflowHistoryService historyService, WorkflowState workflowState = WorkflowState.Draft)
    {
        ...
        _sm = new StateMachine<WorkflowState, WorkflowTrigger>(workflowState, FiringMode.Queued);
    ...
    /// parameterized <see cref="WorkflowTrigger.Return"/> trigger for returning to
    /// earlier stages, plus <see cref="WorkflowTrigger.Cancel"/> to move to
    ...
    private void Configure()
    {
        ...
        _sm.Configure(WorkflowState.Draft)
            .PermitIf(WorkflowTrigger.ForwardToMemberInformationEntry, WorkflowState.MemberInformationEntry, CanStartLodAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.MemberInformationEntry)
            .PermitIf(WorkflowTrigger.ForwardToMedicalTechnician, WorkflowState.MedicalTechnicianReview, CanForwardToMedicalTechnicianAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.MedicalTechnicianReview)
            .PermitIf(WorkflowTrigger.ForwardToMedicalOfficerReview, WorkflowState.MedicalOfficerReview, CanForwardToMedicalOfficerReviewAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.MedicalOfficerReview)
            .PermitIf(WorkflowTrigger.ForwardToUnitCommanderReview, WorkflowState.UnitCommanderReview, CanForwardToUnitCommanderReviewAsync)
        ...
        _sm.Configure(WorkflowState.UnitCommanderReview)
            .PermitIf(WorkflowTrigger.ForwardToWingJudgeAdvocateReview, WorkflowState.WingJudgeAdvocateReview, CanForwardToWingJudgeAdvocateReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.WingJudgeAdvocateReview)
            .PermitIf(WorkflowTrigger.ForwardToAppointingAuthorityReview, WorkflowState.AppointingAuthorityReview, CanForwardToAppointingAuthorityReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.AppointingAuthorityReview)
            .PermitIf(WorkflowTrigger.ForwardToWingCommanderReview, WorkflowState.WingCommanderReview, CanForwardToWingCommanderReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.WingCommanderReview)
            .PermitIf(WorkflowTrigger.ForwardToBoardTechnicianReview, WorkflowState.BoardMedicalTechnicianReview, CanForwardToBoardTechnicianReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.BoardMedicalTechnicianReview)
            .PermitIf(WorkflowTrigger.ForwardToBoardMedicalReview, WorkflowState.BoardMedicalOfficerReview, CanForwardToBoardMedicalReviewAsync)
            .PermitIf(WorkflowTrigger.ForwardToBoardLegalReview, WorkflowState.BoardLegalReview, CanForwardToBoardLegalReviewAsync)
            .PermitIf(WorkflowTrigger.ForwardToBoardAdministratorReview, WorkflowState.BoardAdministratorReview, CanForwardToBoardAdministratorReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
            .PermitIf(WorkflowTrigger.Cancel, WorkflowState.Cancelled, CanCancelAsync)
        ...
        _sm.Configure(WorkflowState.BoardAdministratorReview)
            .PermitIf(WorkflowTrigger.Complete, WorkflowState.Completed, CanCompleteAsync)
            .PermitIf(WorkflowTrigger.ForwardToBoardTechnicianReview, WorkflowState.BoardMedicalTechnicianReview, CanForwardToBoardTechnicianReviewAsync)
            .PermitIf(WorkflowTrigger.ForwardToBoardMedicalReview, WorkflowState.BoardMedicalOfficerReview, CanForwardToBoardMedicalReviewAsync)
            .PermitIf(WorkflowTrigger.ForwardToBoardLegalReview, WorkflowState.BoardLegalReview, CanForwardToBoardLegalReviewAsync)
            .PermitDynamicIf(_returnTrigger, (_, destination) => destination, CanReturnAsync)
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToMemberInformationEntry"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToMedicalTechnician"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToMedicalOfficerReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToUnitCommanderReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToWingJudgeAdvocateReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToWingCommanderReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToAppointingAuthorityReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.Complete"/> trigger may
    ...
    /// are present, and the final LOD finding (<see cref="FindingType"/>) has been
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.Cancel"/> trigger
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToBoardTechnicianReview"/>
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToBoardMedicalReview"/>
    /// trigger may be fired. Used by board-level states
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToBoardLegalReview"/>
    /// trigger may be fired. Used by board-level states
    /// (<see cref="WorkflowState.BoardMedicalTechnicianReview"/>,
    ...
    /// <summary>
    /// Guard that determines whether the <see cref="WorkflowTrigger.ForwardToBoardAdministratorReview"/>
    /// trigger may be fired. Used by board-level states
    /// (<see cref="WorkflowState.BoardMedicalTechnicianReview"/>,
    /// <see cref="WorkflowState.BoardMedicalOfficerReview"/>, and
    ...
    /// <summary>
    /// Guard that determines whether the parameterized <see cref="WorkflowTrigger.Return"/>
    ...
    /// payload (e.g., <see cref="WorkflowTrigger.Cancel"/>).
    /// </summary>
    ...
    /// </returns>
    public async Task<StateMachineResult> FireAsync(WorkflowTrigger trigger)
    {
    ...
    /// index) or failure (with the error message from the failed save operation).
    /// </returns>
    public async Task<StateMachineResult> FireAsync(LineOfDutyCase newCase, WorkflowTrigger trigger)
    {
    ...
    /// <summary>
    /// Fires the parameterized <see cref="WorkflowTrigger.Return"/> trigger, transitioning the
