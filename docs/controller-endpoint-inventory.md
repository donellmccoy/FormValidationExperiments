...
- `MaxExpansionDepth = 3` on all `[EnableQuery]` attributes
...
| Method | Route | OData? | Serialization |
|--------|-------|--------|:---:|
| `Get()` | `GET /odata/Authorities` | ✅ `[EnableQuery]`, `FromODataUri` | ✅ |
...
| `Post` | `POST /odata/Bookmarks` | ✅ `[EnableQuery]`, `Created()` | ✅ |
| `Delete` | `DELETE /odata/Bookmarks({key})` | ✅ `FromODataUri` | ✅ |

## 3. CasesController — All OData — ~~🟡 2 Issues~~ Fixed
...
> ~~🟡 **Checkout / Checkin** — returned `Ok(existing)` from `FindAsync`, serializing a full entity the client never reads. **Fixed** — both now return `NoContent()` (HTTP 204). The client (`CaseService.cs`) only checks `IsSuccessStatusCode` and returns `bool`.~~

## 4. CaseDialogueCommentsController — All OData — ~~🔴 1 Issue~~ Fixed
...
> ~~🔴 **Patch** had `[FromBody]` on `Delta<CaseDialogueComment>`, which forced STJ deserialization. STJ cannot construct `Delta<T>`, so the endpoint was broken at runtime. **Fixed** — `[FromBody]` removed so the OData input formatter handles it.~~
...
> ~~🟡 **Put** — used `SetValues(document)` which copied all scalar properties from the client payload, including audit fields. **Fixed** — audit fields (`CreatedBy`, `CreatedDate`, `ModifiedBy`, `ModifiedDate`) are now marked `IsModified = false` after `SetValues()` to prevent over-posting.~~
>
...
## 6. MembersController — All OData — ~~🟡 1 Issue~~ Fixed
...
> ~~🟡 **Put** — used `SetValues(member)` which copied all scalar properties from the client payload, including audit fields. **Fixed** — audit fields (`CreatedBy`, `CreatedDate`, `ModifiedBy`, `ModifiedDate`) are now marked `IsModified = false` after `SetValues()` to prevent over-posting.~~
...
| Controller | Total | OData | Non-OData | Issues |
|------------|:-----:|:-----:|:---------:|:------:|
| AuthoritiesController | 5 | 5 | 0 | ✅ 0 |
| BookmarksController | 3 | 3 | 0 | ✅ 0 |
| CasesController | 15 | 15 | 0 | ~~🟡 2~~ Fixed |
| CaseDialogueCommentsController | 4 | 4 | 0 | ~~🔴 1~~ Fixed |
| DocumentsController | 8 | 5 | 3 | 🟡 1 (1 Fixed) |
| MembersController | 7 | 7 | 0 | ~~🟡 1~~ Fixed |
| WorkflowStateHistoryController | 4 | 4 | 0 | ✅ 0 |
| UserController | 2 | 0 | 2 | ✅ 0 |
| **Total** | **48** | **43** | **5** | **1 open (5 fixed)** |
