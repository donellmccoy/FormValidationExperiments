...
## Phase 1: Input DTOs in `ECTSystem.Shared/ViewModels/`
...
### 1f. `UpdateCaseDto` — for `CasesController.Patch` with `Delta<UpdateCaseDto>` 🆕

**Why:** PATCH currently uses `Delta<LineOfDutyCase>`, which allows clients to submit changes to any of the entity's 100+ properties — including server-managed fields (`IsDeleted`, `IsCheckedOut`, `CheckedOutBy`, `CheckedOutDate`, `CurrentWorkflowState`, audit fields). Although `Delta<T>` only applies *submitted* fields, a malicious client can include protected fields in the PATCH body. Using `Delta<UpdateCaseDto>` restricts patchable fields at the type level. **(Resolves remediation plan item 9.2.)**
...
**EDM Registration:** `UpdateCaseDto` must be registered as an entity type (or unbound complex type) in the OData EDM model for `Delta<UpdateCaseDto>` model binding to work.

**Controller Pattern:**

```csharp
public async Task<IActionResult> Patch([FromODataUri] int key, [FromBody] Delta<UpdateCaseDto> delta)
{
    var entity = await context.LineOfDutyCases.FindAsync(key);
    if (entity is null) return NotFound();

    var dto = CaseDtoMapper.ToUpdateDto(entity);  // entity → DTO snapshot
    delta.Patch(dto);                               // apply only submitted fields
    CaseDtoMapper.ApplyUpdate(dto, entity);         // map DTO back → entity
    await context.SaveChangesAsync();
    return Updated(entity);
}
```

**Note:** The same `Delta<UpdateXxxDto>` pattern applies to `AuthoritiesController.Patch` and `MembersController.Patch`. If those entities gain sensitive server-managed fields beyond the `AuditableEntity` base, add corresponding Update DTOs. For now, their audit fields are the main exposure.
...
| **CasesController (PATCH)** | ~~Keep as-is~~ → **Now uses `Delta<UpdateCaseDto>`** (see Phase 1f). `Delta<LineOfDutyCase>` exposed server-managed fields to over-posting. |
...
## Phase 4: Mapper Classes in `ECTSystem.Shared/Mapping/`
...
├── CaseDtoMapper.cs                     (🆕 CreateCaseDto → LineOfDutyCase, UpdateCaseDto ↔ LineOfDutyCase)
...
## Phase 8: ETag / If-Match Concurrency Enforcement on PATCH
...
| 4. Update BookmarksController | Low | Low — simplest DTO |
| 5. Update AuthoritiesController | Low | Low |
| 6. Update MembersController | Medium | Low |
| 7. Update CasesController (POST) | Medium | Medium — CaseId generation logic |
| 8. Update CasesController (PATCH → `Delta<UpdateCaseDto>`) | Medium | Medium — EDM registration + mapper |
| 9. Wire `If-Match` ETag on PATCH (Phase 8) | Medium | Medium — client must send header; breaks clients that omit it |
| 10. Update Blazor client services (Phase 6) | Medium | Medium — verify OData client compatibility |
| 11. Update tests (Phase 7) | Medium | Low |
...
- **Over-posting on PATCH** — `Delta<UpdateCaseDto>` restricts patchable fields at the type level; server-managed properties don't exist on the DTO **(remediation item 9.2)**
