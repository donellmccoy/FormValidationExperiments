...
- **Status:** Complete — `IBlobStorageService` abstraction with `AzureBlobStorageService` implementation, `BlobContainerInitializer` background service, DI registration via `AddBlobStorage()`, `DocumentsController` uploads to blob and downloads via SAS redirect, `LineOfDutyDocument.BlobPath` replaces legacy `Content` column.

### D.4 CasesController Delete → Soft Delete ✅

- **Scope:** `Controllers/CasesController.cs`, `Models/LineOfDutyCase.cs`, `Data/Configurations/LineOfDutyCaseConfiguration.cs`, migration `20260416013603_SoftDeleteCase`
- **Rationale:** Current delete loads 10 navigation collections (11 SQL queries) before removing. Replaced hard delete with soft delete — sets `IsDeleted`/`DeletedAt`/`DeletedBy` flags on a single row instead.
- **Source:** Cases characterization, Weakness #7/#8.
- **Status:** Complete — Added `IsDeleted`, `DeletedAt`, `DeletedBy` properties to `LineOfDutyCase`. Global query filter (`HasQueryFilter(e => !e.IsDeleted)`) automatically excludes soft-deleted cases from all EF queries. Filtered index `IX_Cases_IsDeleted` on active rows. Delete endpoint rewritten to set flags instead of `context.Cases.Remove()`, eliminating `IncludeAllNavigations()` and manual child-entity removal.
