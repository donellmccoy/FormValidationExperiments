...
Run the embedded data reset script against the **LOCAL** ECT database (`Server=localhost;Database=ECT;Trusted_Connection=True`) using the **VS Code mssql extension** MCP tools.

## Steps

1. **Connect** to the local ECT database using the saved profile **"ECT - LOCAL"**:

	- Tool: `mssql_connect`
	- Parameters:
	  - `serverName`: `localhost`
	  - `database`: `ECT`
	  - `profileId`: `E9430CEC-B6FF-46F6-B1D0-2A09CA3EC31B`
	- Capture the returned `connectionId`.

2. **Run** the embedded SQL below in a single batch:
...
3. **Verify** the final `SELECT` result shows:
	...
	- Preserved (non-zero): `__EFMigrationsHistory`, `AspNetUsers`, `Members`, `WorkflowStates` (24), `WorkflowTypes` (2), `WorkflowModules` (2).
...
- LOCAL is the developer's own DB; no extra confirmation prompt is required.
